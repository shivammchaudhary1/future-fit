...
import { Career, CareerSchema, type CareerDocument } from "./career.schema.js";
class PaginationDto {
...
class CareersController {
  constructor(
    @InjectModel(Career.name) private readonly careers: Model<CareerDocument>,
  ) {}
  @Get() async list(@Query() query: PaginationDto) {
    ...
    return {
      success: true,
      data: items.map((item) => ({ _id: item._id, ...item.content })),
  ...
  @Get(":slug") async get(@Param("slug") slug: string) {
    ...
    return { success: true, data: { _id: career._id, ...career.content } };
  }
  @Get(":slug/education-path") async education(@Param("slug") slug: string) {
    return { success: true, data: (await this.get(slug)).data.educationPath };
  }
  @Get(":slug/skills") async skills(@Param("slug") slug: string) {
    return { success: true, data: (await this.get(slug)).data.skills };
  }
}
...
class AdminCareersController {
  ...
  @Post() @UseGuards(CsrfGuard) async create(@Body() body: unknown) {
